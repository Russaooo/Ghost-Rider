...
namespace Ashsvp
{
    public class SimcadeVehicleController : MonoBehaviour
    {
        ...
        void AddAcceleration(float accelerationInput)
        {
            ...
            // корректировка - поскольку двигаем с флагом Kinematic=true на RigidBody - не можем применять ускорение на объект
            if (brakeInput < 0.1f && localVehicleVelocity.z < MaxSpeed && !rb.isKinematic)
            {
